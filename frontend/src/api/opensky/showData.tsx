    ...
    return (
        <ul>
            {planes.map((plane: any) => (
                <li key={plane.icao24}>
                    model: {plane.model}| id: {plane.icao24} | cs: {plane.callsign} | from: {plane.country} | long: {plane.longitude} | lat: {plane.latitude} | alt: {plane.altitude} | cap: {plane.heading} | vel: {plane.velocity} |
                </li>
            ))}
        </ul>
    );
