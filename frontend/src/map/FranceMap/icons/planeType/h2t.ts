// Silhouette d'hélicoptère biturbine (H2T) avec 3 pales et effet de rotation
export function createH2TIcon(color: string, size: number, angleDeg: number = 110): ImageData {
  ...
  // --- 3. ROTOR PRINCIPAL EN ROTATION (3 PALES) ---
  ctx.save();
  ctx.rotate((angleDeg * Math.PI) / 180);

  // A. Disque d'effet de balayage (flou circulaire)
  ctx.beginPath();
  ...
  // C. 3 pales nettes espacées de 120°
  for (let i = 0; i < 3; i++) {
    ctx.save();
