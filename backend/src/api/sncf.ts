...
function formatSncfDatetime(now: Date): string {
  const pad = (num: number) => String(num).padStart(2, '0');
  return `${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(now.getDate())}T${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}`;
}

async function fetchDeparturesForGare(gare: { id: string; name: string }, authHeader: string, datetimeSncf: string) {
  try {
...
/**
 * Récupère les prochains départs des gares majeures (dédupliqués par id).
 * Retourne [] en cas d'erreur globale.
 */
export async function fetchSncfDepartures(apiKey: string) {
  try {
    ...
    const promessesGares = GARES_MAJEURES.map((gare) =>
      fetchDeparturesForGare(gare, authHeader, datetimeSncf)
    );

    const résultatsParGare = await Promise.all(promessesGares);
    const tousLesTrains = résultatsParGare.flat();

    return tousLesTrains.filter((train, index, self) =>
...
/**
 * Récupère le geojson des gares de voyageurs (open data SNCF).
 */
export async function fetchSncfGares() {
  try {
...
/**
 * Récupère le geojson des lignes du réseau ferré national (open data SNCF).
 */
export async function fetchSncfRailLines() {
  try {
