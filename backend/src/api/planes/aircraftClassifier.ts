...
interface TypeInfo {
  engines?: number;
  kindHint?: AircraftKind;
  label?: string;
}

const TYPE_TABLE: Record<string, TypeInfo> = {
  // --- Commercial / lignes régulières ---
  ...
  // --- Aviation générale (turbopropulseurs + monomoteurs/bimoteurs légers) ---
  ...
  // --- Hélicoptères militaires connus (armée de l'air/marine/gendarmerie françaises + OTAN) ---
...
export function classifyOpenskyCategory(
  ...
  let kind: AircraftKind = 'unknown';

  switch (category) {
    case 8: kind = 'helicopter'; break;
    case 9: kind = 'glider'; break;
    case 10: kind = 'balloon'; break;
    case 14: kind = 'uav'; break;
    case 16:
    case 17: kind = 'groundVehicle'; break;
    case 4:
    case 5:
    case 6: kind = 'commercial'; break; // Large / High Vortex Large / Heavy : quasi toujours ligne ou cargo
    case 2:
    case 3: kind = 'generalAviation'; break; // Light / Small : pas assez précis pour distinguer jet privé
...
export function classifyAircraft(raw: any): ClassificationResult {
  ...
  const type: string = (raw?.t ?? '').toString().toUpperCase();
  ...
  const isHelicopter =
    isHelicopterCategory || info?.kindHint === 'helicopter' || info?.kindHint === 'militaryHelicopter';
  const isMilitary = isMilitaryFlag || info?.kindHint === 'military' || info?.kindHint === 'militaryHelicopter';

  let kind: AircraftKind = 'unknown';
  if (isGroundVehicle) kind = 'groundVehicle';
  else if (isUav) kind = 'uav';
  else if (isBalloon) kind = 'balloon';
  else if (isGlider) kind = 'glider';
  else if (isHelicopter && isMilitary) kind = 'militaryHelicopter';
  else if (isHelicopter) kind = 'helicopter';
  else if (isMilitary) kind = 'military';
  else if (info?.kindHint === 'privateJet') kind = 'privateJet';
  else if (info?.kindHint === 'commercial') kind = 'commercial';
  else if (category === 'A3' || category === 'A4' || category === 'A5') kind = 'commercial'; // gros porteurs
  else if (category === 'A1' || category === 'A2') kind = 'generalAviation'; // léger, non identifié précisément
